...
class ContactListFragment : Fragment(), MenuProvider {
    ...
    override fun onMenuItemSelected(menuItem: MenuItem): Boolean {
        ...
        if(menuItem.itemId == R.id.action_download){
            ...
            val contact_id = sharedPreferences.getString(getString(R.string.phone), "")
            //perform validation here. Make sure the contact_id is not empty

            //download contact records from the server
            downloadContact(requireContext(), getString(R.string.read_url) + "contact_id=" + contact_id)
        }
        return true
    }
